...
exports.init = function () {
  ...
  // Homey.app.actionHttpGetVariableBetterLogic = new Homey.FlowCardAction('http_get_variable_Logic')
  //   .registerRunListener(onActionHttpGetVariableHomeyLogic)
  //   .register()
  //   .getArgument('homeyVariable')
  //   .registerAutocompleteListener(onHomeyLogicVariableAutocomplete)
...
// HTTP Get JSON or XML request and put in Homey Logic variable flow action
function onActionHttpGetVariableHomeyLogic (args) {
  // return util.genericRequestHelper('A25 action.http_get_variable_Logic', 'get', args, {json: false})
  //   .then(result => util.resolveJsonPath(result.data, args.path))
  //   .then(variable => {
  //     variable = escape(variable).replace(/\//g, '%2F')
  //     util.debugLog('  --> variable result formatted', variable)
  //     // TODO: test
  //     console.log('set variable on homey manager', args.homeyVariable)
  //     let newVariable = args.homeyVariable
  //     newVariable.value = variable
  //     return Homey.Managerlogic.updateVariable({id: args.homeyVariable.id, variable: newVariable})
  //   })
  //   .then(Promise.resolve(true))
}

function onHomeyLogicVariableAutocomplete (value) {
  // TODO: test
  // util.debugLog('homeyVariable.autocomplete', {value: value})
  // console.log('is it a promise?', Homey.ManagerLogic.getVariables())
  // return Homey.ManagerLogic.getVariables().filterSupportedLogicVariables(value)
}
...
function filterSupportedLogicVariables (partialWord) {
  return function (variable) {
    return variable.name.toLowerCase().indexOf(partialWord.toLowerCase()) > -1
  }
}
