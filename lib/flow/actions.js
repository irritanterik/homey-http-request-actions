...
function onActionHttpGetJson (callback, args) {
  util.genericRequestHelper('action.http_get_json', 'get', args, {query: args.data}).then(result => {
...
function onActionHttpGetVariable1 (callback, args) {
  util.genericRequestHelper('action.http_get_variable_1', 'get', args, {query: args.data, json: false})
...
function onActionHttpGetVariableBetterLogic (callback, args) {
  ...
  util.genericRequestHelper('action.http_get_variable_BetterLogic', 'get', args, {json: false})
    .then(result => { return util.resolveJsonPath(result.data, args.path) })
    .then(variable => {
...
function onActionHttpPostForm (callback, args) {
  util.genericRequestHelper('action.http_post_form', 'post', args, {form: args.data}).then(result => {
...
function onActionHttpPostJson (callback, args) {
  util.genericRequestHelper('action.http_post_json', 'post', args, {json: args.data}).then(result => {
    if (result.response.statusCode === 200 ||
        result.response.statusCode === 201 ||
        result.response.statusCode === 202) {
      return callback(null, true)
...
function onActionHttpPostXml (callback, args) {
  util.genericRequestHelper('action.http_post_xml', 'post', args, {headers: {'content-type': 'application/xml'}}, args.data).then(result => {
    if (result.response.statusCode === 200 ||
        result.response.statusCode === 201 ||
        result.response.statusCode === 202) {
      return callback(null, true)
...
function onActionHttpPutJson (callback, args) {
  util.genericRequestHelper('action.http_put_json', 'put', args, {json: args.data}).then(result => {
