...
exports.init = function () {
  ...
  Homey.manager('flow').on('action.http_get_full_json', onActionHttpGetFullJson)
...
// HTTP Get JSON or XML request flow action
function onActionHttpGetFullJson (callback, args) {
  util.genericRequestHelper('A24 action.http_get_full_json', 'get', args, {json: false})
    .then(result => { return util.convertToJson(result.data) })
    .then(json => {
      Homey.manager('flow').trigger('full_json', {json: JSON.stringify(json)}, {trigger: args.trigger})
      callback(null, true)
    })
    .catch(callback)
}
