...
exports.init = function () {
  ...
  Homey.app.triggerHttpGetVariable2 = new Homey.FlowCardTrigger('http_get_variable_2')
    .registerRunListener((args, state) => {
      console.log('Homey.app.triggerHttpGetVariable2', args.trigger.toLowerCase() === state.trigger.toLowerCase())
      return Promise.resolve(args.trigger.toLowerCase() === state.trigger.toLowerCase())
    })
    .register()

  Homey.app.triggerFullJson = new Homey.FlowCardTrigger('full_json')
    .registerRunListener((args, state) => {
      console.log('Homey.app.triggerFullJson', args.trigger.toLowerCase() === state.trigger.toLowerCase())
      return Promise.resolve(args.trigger.toLowerCase() === state.trigger.toLowerCase())
    })
    .register()
