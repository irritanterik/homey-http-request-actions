...
exports.debugLog = function (message, data) {
  ...
  // this.homey.api.realtime('HTTP Log', logLine)
...
exports.resolveJsonPath = function (data, path) {
  return this.convertToJson(data).then(data => {
    const variable = jsonPath({json: data, path: path, wrap: false})
    const variableType = this.type(variable)
    // this.debugLog('  --> data:', data)
    // this.debugLog('  --> jsonPath:', path)
    // this.debugLog('  --> result', {variable: variable, type: variableType})
    if (variableType === 'undefined') return Promise.reject(new Error('Value not found'))
    if (['object', 'array', 'null'].indexOf(variableType) !== -1) {
      // this.debugLog('  --> error: result is an ' + this.type(variable))
      return Promise.reject(new Error('Result from jsonPath has type ' + this.type(variable)))
    }
...
function mergeOptional (dest, src) {
  ...
  try {
    ...
    console.log('Something went wrong merging url options', {source: src, destination: dest})
  }
